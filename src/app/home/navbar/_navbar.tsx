...
export default function Navbar(
    ...
    const socialItems = [{
        url: "https://www.github.com/epicdevler", imgUrl: faGithub, imgAlt: "GitHub Logo", hoverBg: undefined, hoverContentColor: undefined,
    }, {
        url: "https://www.linkedin.com/in/nwadikephilip", imgUrl: faLinkedinIn, imgAlt: "LinkedIn Logo", hoverBg: undefined, hoverContentColor: undefined,
    }, {
    ...
    function handleNavItemClick(event: { currentTarget: any; preventDefault: () => void; }) {        
    ...
    >
        <FullScreenNav onToggle={isToggled} onNavItemClicked={handleNavItemClick} unToggle={handleNavToggle} />
        <Container boxShadow={{ base: 'none', lg: 'md' }} maxW={'container.lg'} rounded={{ base: '0', lg: 'full' }} dropShadow={'md'} bg={'brand'} >
            <Flex alignItems={'center'} flexDirection={'row-reverse'}>
                <HStack>
                    {socialItems.map((item, index) => {
                        return <SocialIcon hoverBg={item.hoverBg} hoverContentColor={item.hoverContentColor} key={index} href={item.url} iconUrl={item.imgUrl}
                            alt={item.imgAlt} />
                    })}
                </HStack>
                <Spacer />
                <HStack hideBelow={"md"}>
                    {navLinks.map((link, index) => {
                        return <NavLink key={link.label}
                        onClick={
                            handleNavItemClick
                        }
                         href={link.href} label={link.label} isFirstChild={index == 0}
                            isActive={fullpageSectionCount === index} />
                    })}
...
export function FullScreenNav({ onNavItemClicked, onToggle, unToggle }: { onToggle: boolean, unToggle: () => void, onNavItemClicked: MouseEventHandler<HTMLParagraphElement> }) {
        ...
        <VStack aria-modal='true' align={'end'} backgroundColor={"brand"} hideFrom={'md'}
            ...
            {navLinks.map(link => {
                return <Box key={link.label} width={'full'} textAlign={'center'}>
                    <NavLink onClick={onNavItemClicked} label={link.label} href={link.href}
